...
class MapPainter extends CustomPainter {
  ...
  void _drawStaticBackground(Canvas canvas, Size size) {
    ...
    if (currentLabel.toUpperCase().contains('1F')) {
      offscreenCanvas.drawRect(
        ...
        Paint()..color = Colors.lightGreen.shade100,
      );
    } else {
      offscreenCanvas.drawRect(
        const Rect.fromLTWH(-5000, -5000, 10000, 10000),
    ...
    // 2. グリッド
    final gridPaint = Paint()..color = Colors.white.withValues(alpha: 0.5)..strokeWidth = 1;
    for (double x = 0; x <= AppConfig.mapCanvasSize; x += 10) {
      offscreenCanvas.drawLine(Offset(x, 0), Offset(x, AppConfig.mapCanvasSize), gridPaint);
    }
    for (double y = 0; y <= AppConfig.mapCanvasSize; y += 10) {
      offscreenCanvas.drawLine(Offset(0, y), Offset(AppConfig.mapCanvasSize, y), gridPaint);
    }
  ...
  void _drawDynamicElements(Canvas canvas, Size size) {
    ...
    // 目的地マーカー
    if (goalNode != null) {
      final p = goalCenter ?? _findNodePos(goalNode!);
      if (p != null) {
        final markerPos = goalCenter != null ? p : Offset(p.dx + 5, p.dy + 5);
    ...
    if (estimatedPosition != null && showUserDot) {
      ...
      canvas.drawCircle(p, 8, Paint()..color = Colors.blue.shade700);
      canvas.drawCircle(p, 4, Paint()..color = Colors.white);
      
      if (headingDeg != null) {
        _drawHeading(canvas, p, headingDeg!);
      }
    }
  }
  ...
  Offset? _findNodePos(String idOrName) {
    if (nodes.containsKey(idOrName)) {
      final n = nodes[idOrName];
      return Offset((n['x'] as num).toDouble(), (n['y'] as num).toDouble());
    }
    for (final n in nodes.values) {
      if (n is Map && n['name'] == idOrName) {
        return Offset((n['x'] as num).toDouble(), (n['y'] as num).toDouble());
      }
    }
    return null;
  ...
  void _drawBorders(Canvas canvas, Map n) {
    ...
    final wallPaint = Paint()..color = Colors.red.shade900..strokeWidth = 2;
    final doorPaint = Paint()..color = Colors.orange.shade800..strokeWidth = 4;

    if (n['wallTop'] == true) canvas.drawLine(Offset(x,y), Offset(x+10,y), wallPaint);
    if (n['wallBottom'] == true) canvas.drawLine(Offset(x,y+10), Offset(x+10,y+10), wallPaint);
    if (n['wallLeft'] == true) canvas.drawLine(Offset(x,y), Offset(x,y+10), wallPaint);
    if (n['wallRight'] == true) canvas.drawLine(Offset(x+10,y), Offset(x+10,y+10), wallPaint);

    if (n['doorTop'] == true) canvas.drawLine(Offset(x+2,y), Offset(x+8,y), doorPaint);
    if (n['doorBottom'] == true) canvas.drawLine(Offset(x+2,y+10), Offset(x+8,y+10), doorPaint);
    if (n['doorLeft'] == true) canvas.drawLine(Offset(x,y+2), Offset(x,y+8), doorPaint);
    if (n['doorRight'] == true) canvas.drawLine(Offset(x+10,y+2), Offset(x+10,y+8), doorPaint);
  }

  Color _getColorForType(int type, String floor) {
    switch (type) {
      case 1: return Colors.white;
      case 3: return Colors.orange.shade50; // ちょっと見やすく
      case 4: return Colors.brown.shade100;
      case 5: return Colors.purple.shade100;
      case 6: return floor.contains('1F') ? Colors.lightGreen.shade200 : Colors.white;
